 ...
 * This code includes an interface of various forms with optionality to perform operations on the forms and
 * the information they contain.
 * The variety of shapes is: circle, square, line, triangle and polygon.
 * The variety of actions that can be performed on the shapes are -
 * move, copy, remove, rotate, increase 1.1, and decrease 0.9.
 * Also, there are different sorting options on the shapes. For example sorting by area.
 * You can also choose a shape by checking if the point is in the shape.
 * Actually, the code is divided into several parts:
 * For each shape, there is an implementation of different functions, some of which are part of the functionality in the interface.
 * For example, like calculating area and height.
 * Also, attached for each form is an implementation of tests that check the correctness of the functions.
 * Beyond that, there is a class that implements an array consisting of a string of attributes of the shapes inserted into the array.
 * On the array there are functions that are also applied to such an array, functions such as adding a specific cell element in the array,
 * removing any element from a specific cell, and more special functions that I will expand on now.
 * The save function converts the shapes to a string with the attributes of each shape.
 * The load function reads a string with attributes of the shapes and converts it to actual shapes.
 *
...
public class Ex2 implements Ex2_GUI {
    ...
    private static void drawShape(GUI_Shape g) {
        ...
        if (gs instanceof Circle_2D) {
            ...
            double rad = c.getRadius();
            if (isFill) {
                StdDraw_Ex2.filledCircle(_p1, cen.y(), rad);
            } else {
                StdDraw_Ex2.circle(cen.x(), cen.y(), rad);
            }
        }
